...
export class ChooseProfessionComponent implements OnInit {
  readonly professionForm: FormGroup = new FormGroup({professionKey: new FormControl()});
  public raceKey: Race['key'] = 'human';


  constructor(@Inject(SET_PROFESSION_COMMAND_PORT) private _setProfessionCommandPort: SetProfessionCommandPort, @Inject(GET_RACE_COMMAND_PORT) private _getRaceCommandPort: GetRaceCommandPort) {
  }
  ...
  professions$: Observable<Profession[]> = of(this.professionsByRace)
    .pipe(map(byRace => byRace.get(this.raceKey)), map(professionKeys => this.professions.filter(profession => professionKeys?.includes(profession.key))))
  ...
  ngOnInit(): void {
    this._getRaceCommandPort.getRace().subscribe((command) => this.raceKey = command.raceKey);
  }
}
