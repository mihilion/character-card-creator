...
@Injectable()
export class CharacterState implements SetRaceCommandPort, GetRaceCommandPort, SetProfessionCommandPort {
  ...
  getRace(): Observable<GetRaceCommand> {
    return this._characterContextPort.getState().pipe(map(state => <GetRaceCommand>{ raceKey: state.raceKey }))
  }
